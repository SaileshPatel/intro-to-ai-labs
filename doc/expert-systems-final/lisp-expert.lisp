...
(defun get-rules-with-conclusion-matching-condition (condition *rules)
  (let ((matching-rules nil)
	(condition-name (first condition)))
    (dolist (rule *rules matching-rules)
      (when (eql condition-name (first rule))
	(push rule matching-rules)))))
...
;;
 (defun backward-chain (rules facts rule &optional rule-chain)
  (let* ((conditions (get-conditions rule))
    ...
    ;; see if the conclusion is in the facts
    (if (conclusion-known rule facts)
	;; put rule on the rule-chain and return rule chain
	(setf rule-chain (cons rule rule-chain))
      ;; else see if conditions of rule are true
      (progn
	(dolist (condition conditions)
	  ;; function to check condition in fact base
	  (unless (condition-true condition facts) 
	    ;; condition doesn't match facts so is unproved
	    (setf unproved-condition condition)
	    ;; try to prove condition by seeing if it is also a rule conclusion, 
	    ;; noting that it can be the rule conclusion for more than one rule
	    (let ((matching-rules (get-rules-with-conclusion-matching-condition condition rules)))
	      (when matching-rules ;; condition matches rule conclusion
		;; backward chain on each rule in turn until one is true or no rules left
		(dolist (rule matching-rules rule-chain)
		  ;; backward chain on the rule and add what it returns to the original rule rule-chain.
		  (setf rule-chain (backward-chain rules facts rule (cons rule rule-chain)))
		  (when rule-chain ;; will be nil if no rule-chain
		    (setf unproved-condition nil)
		    (return))))))) ;; rule proved so condition is also proved and no need to check any further rules
	(if rule-chain 
	    ;; rule is proved so return the rule-chain
	    rule-chain
	  ;; rule not proved and format t returns nil, which is what
	  ;; we want
	  (format t "~%I can't prove the animal is a ~a using the rule
~a
because it does not have ~a with truth status of ~a.~2%"
		    (first rule) rule (first unproved-condition) (second unproved-condition)))))))
...
;; ----------------------------------------------------
;; FUNCTIONS TO GET FACTS FROM THE USER AND REASON AGAIN
;;
;; INCOMPLETE: YOU CAN TRY TO EXPAND ON THESE AND PROVIDE
;; A BETTER USER EXPERIENCE
;;
;; ----------------------------------------------------

;; not been written
(defun get-facts-from-user (rules goals))
 
;; see if rule has any false conditions and return the rule if it is
;; disproved or nil otherwise. 
;;
(defun rule-disproved (rule)
  (let ((disproved nil)) ;; set disproved to be the rule
    (dolist (condition (get-conditions rule))
      (when (and (condition-known condition)
		 (not (condition-true condition)))
	(setf disproved T)
	(return)))
    disproved))

;; Function to get facts from the user if the system does not have
;; enough to prove any rule by forward chaining. Takes a rule and asks
;; facts to ensure it is triggered. 
;;
(defun get-rules-to-pursue (rules)
  (let ((rules-to-pursue nil))
    ;; go through the rules in order
    (dolist (rule rules rules-to-pursue)
      ;; if rule has not got conclusion in the facts and has no false
      ;; conditions, then add to set of rules worth pursuing
      (unless (or (conclusion-known rule)
		  (rule-disproved rule))
	(push rule rules-to-pursue)))))

;; Find the number of unknown conditions in a rule
;;
;; Returns the number
(defun unknown-conditions (rule)
  (let ((n 0)) ;; number of unknown conditions
    (dolist (condition (get-conditions rule) n)
      (unless (condition-known condition)
	(incf n)))))

;; control strategy. Can put anything here, so it can be changed
;; without having to alter anything else in the program. 
;;
;; This particular strategy gets the rule with the least number of
;; unknown conditions. It has not been tested!
;;
(defun get-best-rule (rules)
  ;; lowest number of unknowns initialised to first rule
  (let ((min (unknown-conditions (first rules)))
	(best-rule nil))
    (if (= min 1) ;; can't get less than this
	(setf best-rule (first rules))
      ;; else look through the rest of the rules
      (dolist (rule (rest rules))
	(let ((n (unknown-conditions rule))) ;; number of unknowns for current rule
	  (if (= n 1) ;; can't get any fewer unknowns
	      (progn (setf best-rule rule)
		     (return)) ;; don't look any further
	    ;; else make rule current best if fewer unknowns than previous best
	    (when (< n min)
	      (setf min n
		    best-rule rule))))))
